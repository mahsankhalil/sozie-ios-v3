...
public class PinCodeInputView<T: UIView & ItemType>: UIControl, UITextInputTraits, UIKeyInput {
    ...
    private var changeTextHandler: ((String) -> Void)?
    ...
    public func set(text: String) {
        if PinValidator.isPinCode(text: text, digit: digit) {
            self.text = text
        }
    }

    public func set(changeTextHandler: @escaping (String) -> Void) {
    ...
    private func updateText() {
        items.enumerated().forEach { (index, item) in
            if (0..<text.count).contains(index) {
                ...
                item.itemView.text = text[currentIndex]
            } else {
